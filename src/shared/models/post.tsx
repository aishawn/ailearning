...
// Get a doc from database by slug (slug array joined with "/")
// Returns null if not found — caller should fall back to file system
export async function getDocFromDB(
  slugArray: string[] | undefined
): Promise<Post | null> {
  if (!slugArray || slugArray.length === 0) {
    // root index doc: slug = "index"
    const result = await findPost({ slug: 'index', status: PostStatus.PUBLISHED });
    if (result?.type === PostType.DOC) return result;
    return null;
  }

  const slug = slugArray.join('/');
  const result = await findPost({ slug, status: PostStatus.PUBLISHED });
  if (result?.type === PostType.DOC) return result;
  return null;
}
