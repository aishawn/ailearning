...
// Helper function to check if a route should be excluded from i18n processing
function shouldExcludeFromI18n(pathname: string): boolean {
  // Exclude system routes and invalid routes
  if (
    pathname.startsWith('/_') || // Next.js internal routes
    pathname.startsWith('/api/') || // API routes
    pathname.startsWith('/acme-challenge') || // ACME challenge routes
    pathname.includes('_vti_bin') || // SharePoint routes
    pathname.includes(':') || // Routes with port numbers
    pathname.match(/^\/\d+$/) || // Pure numeric routes
    pathname.includes('..') // Invalid paths
  ) {
    return true;
  }
  return false;
...
export async function proxy(request: NextRequest) {
  ...
  // Skip i18n processing for excluded routes
