...
export class FalProvider implements AIProvider {
  ...
  private formatInput({
    ...
    if (options.image_input && Array.isArray(options.image_input)) {
      ...
      if (['fal-ai/kling-video/o1/video-to-video/edit'].includes(model)) {
        ...
        // nano-banana models expect 'image_urls' parameter (array format)
        // Ensure image URL is accessible and properly formatted
        input.image_urls = [this.validateImageUrl(imageUrl)];
      } else {
        input.image_url = this.validateImageUrl(imageUrl);
      }
  ...
  // Validate and potentially proxy image URL for FAL compatibility
  private validateImageUrl(imageUrl: string): string {
    try {
      // Check if it's a Twitter/X media URL which might have access restrictions
      if (imageUrl.includes('pbs.twimg.com') || imageUrl.includes('twitter.com')) {
        console.log('Detected Twitter image URL, may need proxying:', imageUrl);
        // For now, try direct URL but log warning
        // TODO: Consider implementing image proxying/re-uploading service
      }
