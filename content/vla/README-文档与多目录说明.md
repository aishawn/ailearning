...
访问 `/vla`（以及 `/vla/某分类/某文章`）能打开 `content/vla/` 下对应 md 文件，整体流程如下。
...
- 构建时（如 `next dev` / `next build`）会扫描 `content/vla` 下的 `.md`/`.mdx`，在项目根目录生成 **`.source`**（元数据、目录树等）。
- URL 路径与文件路径对应关系：  
  `/vla/VLA_世界模型/小鹏汽车提出 Drive-JEPA...`
  → 文件 `content/vla/VLA_世界模型/小鹏汽车提出 Drive-JEPA....md`（按文件名匹配）。
...
- **侧边栏**：同目录下的 **`layout.tsx`** 使用 `source.pageTree[lang]`，树形结构来自 `content/vla` 的目录与文件结构。

因此：**你访问的 /vla 路径 = content/vla 下的相对路径**，文章来自「数据库文档」或「content/vla 下的 md 文件」。
...
```ts
export const docs = defineDocs({ dir: 'content/vla' });
...
1. **/vla 能访问 content/vla 下的文章**：由 `source.config.ts` 的 docs 源 + `source.ts` 的 docsSource + `(docs)/vla/[[...slug]]/page.tsx` 和 layout 的 pageTree 一起实现。  
