...
- Do **not** leave any default template image paths in page content, especially:
  ...
  - any other `/imgs/features/*` AIShawn screenshots
- Also do **not** keep template default `public/logo.svg` / `public/favicon.ico` as-is; they must be replaced (see logo step).

## Where images are configured (AIShawn Two)
...
- `public/logo.svg` and `public/favicon.ico` are **new** and match the **brandName** (see `references/05-logo-favicon.md`).
